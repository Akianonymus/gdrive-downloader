...
_download_file() {
    ...
    if [ -s "${name_download_file}" ]; then
        ...
        range_download_file="Range: bytes=0-${server_size_download_file}"
    ...
    if [ -n "${OAUTH_ENABLED}" ]; then
        ...
        curl -Ls ${CURL_SPEED} \
            -H "${range_download_file}" \
            -H "Authorization: Bearer ${ACCESS_TOKEN}" \
            "${API_URL}/drive/${API_VERSION}/files/${file_id_download_file}?alt=media&supportsAllDrives=true&includeItemsFromAllDrives=true" >> "${name_download_file}" &
        pid="${!}"
        ...
        curl -Ls ${CURL_SPEED} \
            ...
            "${API_URL}/drive/${API_VERSION}/files/${file_id_download_file}?alt=media&supportsAllDrives=true&includeItemsFromAllDrives=true&key=${API_KEY}" >> "${name_download_file}" &
        pid="${!}"
    else
        # normal downloading
        ...
        curl -L -s ${CURL_SPEED} \
            -H "${range_download_file}" \
            -b "${TMPFILE}"COOKIE \
            "https://drive.google.com/uc?export=download&id=${file_id_download_file}${confirm_string:+&confirm=${confirm_string}}" >> "${name_download_file}" &
        pid="${!}"
    fi
