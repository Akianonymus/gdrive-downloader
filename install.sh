...
_variables() {
    ...
    VALUES_LIST="REPO COMMAND_NAME INSTALL_PATH TYPE TYPE_VALUE SHELL_RC LAST_UPDATE_TIME AUTO_UPDATE_INTERVAL INSTALLATION"
    VALUES_SHA="GDL_SCRIPT_SHA DOWNLOAD_UTILS_SHA COMMON_UTILS_SHA"

    VALUES_REGEX="" && for i in VALUES_LIST VALUES_SHA ${VALUES_LIST} ${VALUES_SHA}; do
...
_download_files() {
    ...
    while read -r line <&4; do
        ...
        case "${local_file}" in
            gdl."${INSTALLATION}")
                [ "${GDL_SCRIPT_SHA}" = "${sha}" ] && continue
                GDL_SCRIPT_SHA="${sha}" && local_file="${COMMAND_NAME}"
                ;;
            download-utils."${INSTALLATION}")
                [ "${DOWNLOAD_UTILS_SHA}" = "${sha}" ] && continue
                DOWNLOAD_UTILS_SHA="${sha}"
                ;;
            common-utils."${INSTALLATION}")
                [ "${COMMON_UTILS_SHA}" = "${sha}" ] && continue
                COMMON_UTILS_SHA="${sha}"
                ;;
        esac
...
_inject_values() {
    ...
    {
        printf "%s\n" "${shebang}"
        for i in VALUES_LIST VALUES_SHA ${VALUES_LIST} ${VALUES_SHA}; do
            printf "%s\n" "${i}=\"$(eval printf "%s" \"\$"${i}"\")\" # added values"
        done
        printf "%s\n" "LATEST_INSTALLED_SHA=\"${LATEST_CURRENT_SHA}\" # added values"
        printf "%s\n" "UTILS_FOLDER=\"${INSTALL_PATH}\" # added values"
...
_uninstall() {
    ...
    rm -f "${INSTALL_PATH}"/"${COMMAND_NAME}" \
        "${INSTALL_PATH}"/download-utils."${INSTALLATION}" "${INSTALL_PATH}"/common-utils."${INSTALLATION}"
    [ -z "$(find "${INSTALL_PATH}" -type f)" ] && rm -rf "${INSTALL_PATH}"
...
_setup_arguments() {
    ...
    [ -z "${INSTALL_PATH}" ] && {
        INSTALL_PATH="$(cd "${_INSTALL_PATH}" && pwd)" || exit 1
        { printf "%s\n" "${PATH}" | grep -q -e "${INSTALL_PATH}:" -e "${INSTALL_PATH}/:" && IN_PATH="true"; } || :
        [ -n "${INSTALL_PATH##${HOME}*}" ] && printf "%s\n" "Error: Install path should be within current user home dir." && exit 1
    }
...
main() {
    ...
    _check_existing_command() {
        if COMMAND_PATH="$(command -v "${COMMAND_NAME}")"; then
            if SCRIPT_VALUES="$(grep -E "${VALUES_REGEX}|^LATEST_INSTALLED_SHA=\".*\".* # added values|^UTILS_FOLDER=\".*\".* # added values" "${COMMAND_PATH}")" &&
                eval "${SCRIPT_VALUES}" && [ -n "${LATEST_INSTALLED_SHA:+${UTILS_FOLDER}}" ]; then
                return 0
            else
