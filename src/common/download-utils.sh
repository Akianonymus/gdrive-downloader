...
_cookies_and_document_stuff() {
    ...
    printf '%s\n' '' >> "${TMPFILE}_${file_id_download_file}_misc"
...
_common_stuff() {
    ...
    if [[ -n "${OAUTH_ENABLED:-${API_KEY_DOWNLOAD}}" ]]; then
        ...
        if [[ -n "${OAUTH_ENABLED}" ]]; then
            ...
            flag_value_download_file="${5:-}"
            if [[ -n "${flag_value_download_file}" ]]; then
        ...
        case "${mime_download_file}" in
            ...
            *) # normal downloading
                ...
                [[ -z "${confirm_string}" ]] && {
                    confirm_string="$(_tmp="$(grep 'name="confirm"' "${TMPFILE}_${file_id_download_file}_misc")" && _tmp="${_tmp##*name=\"confirm\" value=\"}" && printf "%s" "${_tmp%%\"*}")"
                }
