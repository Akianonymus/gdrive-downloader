...
"
    ...
    ###################################################
    _set_help() {
        declare content1="${3}" content2 all_content
        [[ -n ${4} ]] && {
...
EOF
        ...
        # now concatenate all the help contents
        all_content="${start}${content1}${content2}"
        for f in ${1}; do
            declare flag="${f//-/}"
            _set_value d "help_${flag}" "${all_content}"
        done
...
${all_content}"
    ...
    [[ -n ${1} ]] && {
        declare tmp_help
        _set_value i tmp_help "help_${1//-/}"

        if [[ -z ${tmp_help} ]]; then
            printf "%s\n" "Error: No help found for ${1}"
        else
            printf "%s\n%s\n%s\n" "${_HELP_BAR}" "${tmp_help}" "${_HELP_BAR}"
        fi
        exit 0
    }
...
_setup_arguments() {
    ...
    declare ARIA_extra_flags="" CURL_extra_flags=""
...
main() {
    ...
    [[ -z ${SELF_SOURCE} ]] && {
        ...
        { . "${COMMON_UTILS_FILE}" &&
            . "${UTILS_FOLDER}"/auth-utils.bash &&
            . "${UTILS_FOLDER}"/common-utils.bash &&
            . "${UTILS_FOLDER}"/drive-utils.bash &&
            . "${UTILS_FOLDER}"/download-utils.bash; } ||
